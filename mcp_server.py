...
def search_knowledge(query: str, k: int = 5) -> list[dict]:
    """
    Performs a semantic vector search over indexed fact chunks to return up to 'k' ranked results with provenance.
    Use this tool instead of re-fetching URLs or re-reading source files if the corpus has already been indexed in Memory. Because this relies on embedding-based search, it evaluates semantic similarity; if an exact word match fails to yield results, try searching with synonyms or related concepts.
    Example:
        search_knowledge("authentication flow", 5)
    """
